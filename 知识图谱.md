...
> **总学时**：21 学时（7 个模块各一个上午、3h），连续 7 天完成
> **生成时间**：2026-07-13
...
| 课程结构 | 模块 0-6 共 7 个模块，各一个上午、3h；14 个 notebook + 1 个 pipeline.ipynb |
...
> **视图目的**：直观呈现连续 7 天、每天一个3小时上午的模块节奏与主线项目推进。
> **受众**：培训者备课、课程评估
> **数据校验源**：[readme.md §四时间规划](readme.md)

```mermaid
flowchart TB
  subgraph 模块0["模块 0 Python 基础 1个上午 3h"]
    W1A["上午课程<br/>基础语法 + OOP + 调试 Git"]
  end
  subgraph 模块1["模块 1 Linux + 环境 1个上午 3h"]
    W2A["上午课程<br/>Linux + 环境搭建 + Jupyter 验证"]
  end
  subgraph 模块2["模块 2 DL 入门 1个上午 3h"]
    W3A["上午课程<br/>MLP + CNN + 训练技巧"]
  end
  subgraph 模块3["模块 3 音频分类 1个上午 3h"]
    W4A["上午课程<br/>音频特征 + CRNN + CI 任务"]
  end
  subgraph 模块4["模块 4 DeepACE 1个上午 3h"]
    W5A["上午课程<br/>ACE + 论文 + 代码 + 修改假设"]
  end
  subgraph 模块5["模块 5 DeepFilterNet 1个上午 3h"]
    W6A["上午课程<br/>SE + 论文 + 推理 + CI 整合"]
  end
  subgraph 模块6["模块 6 ASR 1个上午 3h"]
    W7A["上午课程<br/>ASR + Whisper + Pipeline 整合"]
  end
...
|------|------|------|------|-------------|------------|
| 模块 0 | 上午 | Python基础、面向对象、调试与 Git | 3h | 能读懂 | 能写基本数据处理脚本，理解类/模块并完成 Git 提交 |
| 模块 1 | 上午 | Linux、环境搭建与 Jupyter 验证 | 3h | 能读懂 | Linux 上运行 Python，环境就绪 |
| 模块 2 | 上午 | MLP、CNN 与训练技巧 | 3h | 能修改 | 训练语音/噪声二分类并记录实验 |
| 模块 3 | 上午 | 音频特征、CRNN 与 CI 分类任务 | 3h | 能修改→能组合 | 提取梅尔语谱图，完成 CRNN/VAD 练习 |
| 模块 4 | 上午 | ACE、DeepACE论文、代码与修改假设 | 3h | 能组合 | 理解 ACE 编码，运行 DeepACE 推理 |
| 模块 5 | 上午 | 语音增强、DeepFilterNet、推理与 CI 整合 | 3h | 能组合 | 运行 SE 推理，完成增强→ACE 演示 |
| 模块 6 | 上午 | ASR、Whisper 与端到端 Pipeline 整合 | 3h | 能构建 | **完整 pipeline 跑通** |
...
|------|------|------|------|---------|---------|--------------|---------------|
| 0 | 上午 | Python 基础、OOP、调试 Git | 3h | 读懂 | 数据处理脚本与 Git 提交 | 变量/控制流/函数/class/pdb/Git | `01-basics-signal.ipynb`、`02-oop-audio.ipynb`、`03-debugging-git.ipynb` |
| 1 | 上午 | Linux、环境搭建、Jupyter 验证 | 3h | 读懂 | Linux 运行 Python，环境就绪 | cd/ls/SSH/tmux/Conda/PyTorch/Docker | `01-linux-survival.ipynb`、`02-environment-setup.ipynb` |
| 2 | 上午 | MLP、CNN、训练技巧 | 3h | 修改 | 语音/噪声二分类与实验记录 | nn.Linear/MLP/Conv2d/MelSpectrogram/SpecAugment | `01-linear-regression.ipynb`、`02-mlp-pytorch.ipynb`、`03-cnn-audio.ipynb`、`04-training-tricks.ipynb` |
| 3 | 上午 | 音频特征、CRNN、CI 相关实战 | 3h | 修改→组合 | CRNN/VAD 练习 | MFCC/梅尔-电极映射/Dataset/CRNN/VAD | `01-audio-features.ipynb`、`02-crnn-classifier.ipynb`、`03-ci-tasks.ipynb` |
| 4 | 上午 | ACE、DeepACE论文、代码与修改假设 | 3h | 组合 | DeepACE 推理与修改方案 | ACE N-of-M/Encoder/损失变体 | `01-ace-and-paper.ipynb`、`02-code-analysis.ipynb`、`03-modification-experiments.ipynb` |
| 5 | 上午 | SE、DeepFilterNet、CI 整合 | 3h | 组合 | 增强→ACE 演示 | PESQ/STOI/ERB/init_df/Enhance→Encode | `01-se-enhancement-basics.ipynb`、`02-code-analysis.ipynb`、`03-ci-integration.ipynb` |
| 6 | 上午 | ASR、Whisper 与 Pipeline | 3h | 构建 | **完整 pipeline 跑通** | WER/CER/CTC/Whisper/SNR 扫描 | `01-asr-principles.ipynb`、`02-whisper-pipeline.ipynb` |
...
| 维度 | 数量 |
|------|------|
| 总课时 | 7 个模块 × 3h = 21h（连续 7 天、每天一个上午） |
| 模块数 | 7 个（模块 0-6） |
...
| 维度 | 《深度学习》课程 | 本培训课程 |
|------|----------------|-----------|
| 学时 | 54 学时（理论 18 + 实验 36） | 21 学时（7 个模块各一个上午、3h） |
