...
**用途：** 模块3上午中段的补充实验（如课堂时间允许），用于对比环境声分类与语音命令分类的差异；并作为上午后段 VAD 实验的基础数据来源——从中抽取语音片段作为正样本。
...
**用途：** 模块3上午后段 VAD 模型训练、模块2主线项目延伸。此数据集是课程主线项目的基础——语音增强前的"检测"环节。
...
**用途：** 模块3上午后段实战——带噪语音可懂度分类。输入一段带噪语音，模型判断"CI用户能否听懂"。
...
1. **先下载 ESC-50** — 最小（约60MB），下载快，模块3上午前、中段的课堂实验都需要它
